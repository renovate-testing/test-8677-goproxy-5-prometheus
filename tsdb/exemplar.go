...
func NewCircularExemplarStorage(len int, reg prometheus.Registerer) (ExemplarStorage, error) {
	...
	c := &CircularExemplarStorage{
		...
		lastExemplarsTs: prometheus.NewGauge(prometheus.GaugeOpts{
			...
			Help: "The timestamp of the last appended exemplar stored in circular storage. Useful to check for what time" +
				"range current exemplar buffer limit allow. This usually means the last timestamp" +
				"for all exemplars for typical setup, but not if one series timestamps is in future ",
		}),
